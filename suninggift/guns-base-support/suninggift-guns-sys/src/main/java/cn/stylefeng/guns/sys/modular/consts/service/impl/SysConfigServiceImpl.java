...
//    private Map<String, String> map = new HashMap<>();
    ...
    public String getByCode(String code) {
        if(null == redisUtil.get(systemConfig)){
            //如果为空
            this.flushConfig();
        }
        Map<String, String> map = (Map) redisUtil.get(systemConfig);
        return map.get(code);
    }

    @Override
    public String flushConfig() {
        List<SysConfig> sysConfigs = this.baseMapper.selectList(null);
        Map<String, String> map = new HashMap<>();
        for(SysConfig conf : sysConfigs) {
            map.put(conf.getCode(), conf.getValue());
        }
        redisUtil.set(systemConfig, map, -1);
